...
<!-- Suppression forcée du scroll-to-top + Fix espace blanc mobile -->
<script>
document.addEventListener('DOMContentLoaded', function() {
    // Supprime le bouton scroll-to-top s'il existe
    var scrollBtn = document.querySelector('.scroll-to-top');
    if (scrollBtn) {
        scrollBtn.remove();
    }

    // FIX ESPACE BLANC MOBILE: Détection et suppression automatique
    function fixMobileWhitespace() {
        if (window.innerWidth <= 768) {
            // Force la hauteur automatique sur tous les containers principaux
            const containers = [
                '.main-content',
                '.dashboard-page', 
                '.dashboard-container',
            ...
            // Force body sans espace excédentaire
    ...
    // Applique le fix au chargement ET au resize
    fixMobileWhitespace();
    window.addEventListener('resize', fixMobileWhitespace);

    // ...existing code mobile menu...
